...
\begin{enumerate}  
	...
	\item The decision rules are extracted from the model tree. For curiosity the 10-fold Cross Validation technique is applied on the rules configuration (table \ref{tab:CV-Rules}) to compare the measures with those of the trees (model and regression). The Mean Square Error (MSE) is slighty higher although the number of rules used are more than halved.
\end{enumerate}
...
M5PrimeLab Octave toolbox has been choosen.
\begin{enumerate}  
	\item The tree forest also called ensemble is build with suggested parameters. Customization of parameters has been necessary to lower as much as possible the Mean Square Error (MSE). The error rate is plotted using different numbers of random chosen features (figure \ref{fig:MSE}). The number of used trees is set to 50 for obtaining the best tradeoff between metrics and running times. Growing big ensembles is very expensive. For this reason higher values of 50 are not taken into consideration. 
	\item The variable importance (influence) is plotted (figure \ref{fig:Var-Importance}) In an ensembles, in contrast do the single decision tree, a greater number of variables contribute to the result because different combinations of features are used.
	\item Cross Validation folding technique is used to evaluate the predictive performance of the forest. The best MSE is obtained with K=10. Higher values cause a exponential growth in the algorithm running times. As seen in the graph Cross validation MSE is lower than the Out-of-bag MSE (figure \ref{fig:CV}).
\end{enumerate}
...
\begin{enumerate}  
	...
	\item Plot the learning curve for Gaussian and Bernoulli Naive Bayes classifier
\end{enumerate}
...
\begin{table}[H]
	\centering
	\caption{Results comparison using 432 test records (33\% of dataset)}
