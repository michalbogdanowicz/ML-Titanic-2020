...
\subsection{Procedure: Decision Tree}
M5PrimeLab Octave toolbox has been choosen.
\begin{enumerate}  
	...
	\item Different values for K are tried to run Cross Validation. Various experiments has showed K=10 is ideal for best tradeoff between metrics and algorithm running times. The collected measures are MAE, MSE, RMSE, RRMSE, R2, nRules, nVars.
	...
	\item The model tree prediction value has been calculated, the training set mean and the input variable contributions. The top three variables influencing the model are: Title (Mr.), Pclass 3 (lower class) and Sibsp (number of siblings/spouses).
	\item The decision rules are extracted from the model tree. For curiosity the 10-fold Cross Validation technique is applied on the rules configuration to compare the resulting measures with those of the tree (model and regression). The Mean Square Error (MSE) is slighty higher although the number of rules used are more than halved.
\end{enumerate}

\subsection{Procedure: Random Forest}
M5PrimeLab Octave toolbox has been choosen.
\begin{enumerate}  
	\item The tree forest also called ensemble is build with suggested parameters. Customization of parameters has been necessary to lower as much as possible the Mean Square Error (MSE). The error rate is plotted using different numbers of random chosen features. The number of used trees is set to 50 for obtaining the best tradeoff between metrics and running times. Growing big ensembles is very expensive. For this reason higher values of 50 are not taken into consideration. 
	\item The variable importance (influence) is plotted. In an ensembles, in contrast do the single decision tree, a greater number of variables contribute to the result because different combinations of features are used.
	\item Cross Validation folding technique is used to evaluate the predictive performance of the forest. The best MSE is obtained with K=10. Higher values cause a exponential growth in the algorithm running times. As seen in the graph Cross validation MSE is lower than the Out-of-bag MSE.
\end{enumerate}
...
\subsection{Procedure}
Python Scikit-learn library has been choosen.
\begin{enumerate}  
	\item The Pearson correlation coefficient (PCC) on each pair of features is plotted to inspect the dependencies between features. Naive Bayes classifier requires independent features to get the best results.
	\item The three types of Naive Bayes classifier (Gaussian, Bernoulli, Multinomial) are used on the test set (30\% of dataset) to inspect the incorrect prediction results. Bernoulli Naive Bayes achieved the lowest error rate.
	\item The model metrics using the Gaussian Naive Bayes are calculated and the probability of each class (dead or survived) is predicted.
	\item Plot the learning curve for Gaussian classifier and Bernoulli Naive Bayes classifier
\end{enumerate}
...
\begin{table}[H]
	\centering
	\caption{Results comparison using 432 test records (30\% of dataset)}
...
\begin{table}[H]
	\centering
	\caption{Model metrics with the remaining 70\% of train data}
...
\begin{figure}[H]
	...
	\caption{Gaussian Naive Bayes: the learning curve, scalability and performace. Right part is with scaled data.}
\end{figure}

\begin{figure}[H]
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{bernoulliNB_curve.png}
	\caption{Bernoulli Naive Bayes: the learning curve, scalability and performace. Right part is with scaled data.}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Running on the chosen model the learning curve, scalability and performace. Same data one, left side is not scaled.}
\end{figure}
