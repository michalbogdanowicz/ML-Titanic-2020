...
The model is based on linear regression. The weights can be printed as too have an inclination on to why
...
E.g why do 2 HL with 28 nodes yield similar result to 1 GL with 3 nodes? (there is a difference of 0.004 accuracy) Could it be that the linear combination on the first layer is enough to predict correctly most of survivals? And a second layer finds the non-linear correlation between the survival and the features? Again why 4 nodes perform worse than 3? \\
The positive outcome is based on the fact that neural networks are able to approximate any funcion. Which means that there must be some correlation between the features and the survival of a person.
...
\subsection{Outcome explanation}
Decision trees do not need to normalize or scale feature to work well. They are suitable fo mixed feature data types and the result interpretation of small depth trees is easy. However, they tend to overfit data (high depth trees)  and cause higher variance in the model.
\break Random forests solve the overfitting problem due to the average result gained from the tree ensemble. However, with high-dimensional data scalability problems arise.
